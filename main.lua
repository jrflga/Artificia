...
function remove_by_value(table, value)
    j = 0
    ...
    while #open_list not 0
        ...
        if node_current.x not 0 then
            ...
            insert(nodes_around, {
                ...
                y = left.y,
                h = h_score(left, goal),
                g = g_score(node_current.g, "horizontal"),
                f = h_score(left, goal) + g_score(node_current.g, "horizontal")
            })
            if node_current.y not 0 then
                local top_left = tilemap[node_current.x - 1][node_current.y - 1]
                insert(nodes_around, {
                    x = top_left.x,
                    y = top_left.y,
                    h = h_score(top_left, goal),
                    g = g_score(node_current.g, "diagonal"),
                    f = h_score(top_left, goal) + g_score(node_current.g, "diagonal")
                })
            end
            if (node_current.y + 1) <= mapsizeY - 1 then
                local bottom_left = tilemap[node_current.x - 1][node_current.y + 1]
                insert(nodes_around, {
                    x = bottom_left.x,
                    y = bottom_left.y,
                    h = h_score(bottom_left, goal),
                    g = g_score(node_current.g, "diagonal"),
                    f = h_score(bottom_left, goal) + g_score(node_current.g, "diagonal")
                })
            end
        end
        if node_current.y not 0 then
            ...
            insert(nodes_around, {
                ...
                y = top.y,
                h = h_score(top, goal),
                g = g_score(node_current.g, "vertical"),
                f = h_score(top, goal) + g_score(node_current.g, "vertical")
            })
        end
        if (node_current.x + 1) <= mapsizeX - 1 then
            ...
            insert(nodes_around, {
                x = right.x,
                y = right.y,
                h = h_score(right, goal),
                g = g_score(node_current.g, "horizontal"),
                f = h_score(right, goal) + g_score(node_current.g, "horizontal")
            })
            if node_current.y not 0 then
                local top_right = tilemap[node_current.x + 1][node_current.y - 1]
                insert(nodes_around, {
                    x = top_right.x,
                    y = top_right.y,
                    h = h_score(top_right, goal),
                    g = g_score(node_current.g, "diagonal"),
                    f = h_score(top_right, goal) + g_score(node_current.g, "diagonal")
                })
            end
            if (node_current.y + 1) <= mapsizeY - 1 then
                local bottom_right = tilemap[node_current.x + 1][node_current.y + 1]
                insert(nodes_around, {
                    x = bottom_right.x,
                    y = bottom_right.y,
                    h = h_score(bottom_right, goal),
                    g = g_score(node_current.g, "diagonal"),
                    f = h_score(bottom_right, goal) + g_score(node_current.g, "diagonal")
                })
            end
        end
        if (node_current.y + 1) <= mapsizeY - 1 then
            ...
            insert(nodes_around, {
                x = bottom.x,
                y = bottom.y,
                h = h_score(bottom, goal),
                g = g_score(node_current.g, "vertical"),
                f = h_score(bottom, goal) + g_score(node_current.g, "vertical")
            })
        end

        for j=0, #nodes_around do
            if has_xy(open_list, nodes_around[j]) then
                if node_current.f >= nodes_around[j].g then
                    nodes_around[j].g = node_current.f
                    nodes_around[j].parent = {
                        x = node_current.x,
                        y = node_current.y
                    }
                end
            elseif has_xy(closed_list, nodes_around[j]) then
                if node_current.f >= nodes_around[j].g then
                    nodes_around[j].g = node_current.f
                    nodes_around[j].parent = {
                        x = node_current.x,
                    ...
                    insert(open_list, nodes_around[j])
                    remove_by_value(closed_list, nodes_around[j])
                end
            end
        end
